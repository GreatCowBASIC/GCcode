...
![Screenshot of the Memory Inspector](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/raw/HEAD/media/memory-inspector-periodic-update-screenshot.png)
...
![Screenshot of the Memory Inspector](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/raw/HEAD/media/memory-inspector-debug-sessions-screenshot.png)
...
Check our [contribution guidelines](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/blob/HEAD/CONTRIBUTING.md) for more info.
...
The primary entry point for the backend functionality of the plugin is the the [`MemoryProvider`](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/blob/HEAD/src/plugin/memory-provider.ts) class. That class
...
register custom capabilities, the [`AdapterRegistry`](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/blob/HEAD/src/plugin/adapter-registry/adapter-registry.ts) matches debug types to objects implementing
the [`AdapterCapabilities`](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/blob/HEAD/src/plugin/adapter-registry/adapter-capabilities.ts) interface.

### Memory Widget

The [`MemoryWidget`](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/blob/HEAD/src/webview/components/memory-widget.tsx) is a wrapper around two functional widgets, a `MemoryOptionsWidget` and a`MemoryTableWidget`.
The [`OptionsWidget`](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/blob/HEAD/src/webview/components/options-widget.tsx) is responsible for configuring the display and fetching memory, and the
[`MemoryTable`](https://github.com/eclipse-cdt-cloud/vscode-memory-inspector/blob/HEAD/src/webview/components/memory-table.tsx) renders the memory according to the options specified by the user in the options.
